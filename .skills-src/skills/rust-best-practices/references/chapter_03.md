...
* `cargo flamegraph` (`flamegraph-rs/flamegraph` on GitHub) — a powerful profiler for Rust code. For MacOS, `samply` (`mstange/samply` on GitHub) might be a better DX option.
