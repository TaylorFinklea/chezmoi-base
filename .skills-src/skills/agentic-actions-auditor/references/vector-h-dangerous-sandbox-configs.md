...
Two actions with dangerous configurations (from research Example 8):
...
- **Default configurations:** Actions without explicit sandbox/safety config fields -- defaults are generally safe (Claude defaults to restricted tools, Codex defaults to `sandbox: workspace-write`)
